...
class MaxCDNTests(unittest.TestCase):
    ...
    def test_get_content_length(self):
        ...
        self.assertEqual(self.maxcdn._get_content_length({"a": "b"}), 3)
        self.assertEqual(self.maxcdn._get_content_length({"a": "b", "c": "d"}), 7)
        self.assertEqual(self.maxcdn._get_content_length({"a": "b b", "c": "d"}), 11)

    def test_get_content_length_header(self):
        self.assertEqual(self.maxcdn._get_content_length_header(None), {"Content-Length": "0"})
        self.assertEqual(self.maxcdn._get_content_length_header({"a": "b"}),
                {"Content-Length": "3"})
    def test_get(self):
    ...
    def test_put(self):
        ...
        self.maxcdn._response_as_json.assert_called_with("put", "/put.json",
                override_headers=True, data={})

        self.maxcdn.put("/put.json", data={ "bah": "boo" })
        self.maxcdn._response_as_json.assert_called_with("put", "/put.json",
                override_headers=True, data={ "bah": "boo" })

        self.maxcdn.put("/put.json", data={ "bah": "boo" }, override_headers=False)
        self.maxcdn._response_as_json.assert_called_with("put", "/put.json",
                data={ "bah": "boo" }, override_headers=False)
    ...
    def test_response_as_json_basics(self):
        ...
        for meth in [ "get", "post", "put", "patch", "delete" ]:
            ...
            self.assertEqual(getattr(self.maxcdn, meth)("/%s.json" % meth),
                    { "res": meth })
    ...
    def test_response_as_json_with_files(self):
        ...
        for meth in [ "delete" ]:
            ...
            self.assertEqual(getattr(self.maxcdn, meth)("/%s.json" % meth,
                    data={"req": meth}), { "res": meth })

    def test_response_as_json_pruge(self):
        self.maxcdn._response_as_json = self.was_response_as_json
        requests.Session.request = mock.create_autospec(mock_request,
                return_value=response(json={ "res": "delete" }))
